"""Stage 1.1 — TL1.zip → train/val/test.jsonl + parse_stats.json.

Per docs/witness_train_directive_1.md §1.

Run (Python 3.11):
    "C:/Program Files/Python311/python.exe" -m scripts.witness_train.stage1_1_parse
"""
...
OUT_DIR = ROOT / "data" / "processed" / "witness_v1"
...
def stream_pairs() -> list[dict]:
    """TL1.zip → list of (summary1, passage) pair dicts (drama only)."""
    pairs: list[dict] = []
    skipped = {"not_drama": 0, "empty_summary": 0, "empty_passage": 0}
    with zipfile.ZipFile(TL1_ZIP) as z:
        ...
        for i, name in enumerate(names):
            ...
            if doc_type not in DOC_TYPES:
                skipped["not_drama"] += 1
                continue
            summary1 = (ann.get("Summary1") or "").strip()
...
def _origin_base(doc_origin: str) -> str:
    """작품명 정규화 — drama_mining/data/preprocess.py의 extract_origin_base 그대로.

    예: '장밋빛인생024' → '장밋빛인생', '수상한삼형제15' → '수상한삼형제'.
    회차 suffix (1-3 digit)를 제거. 작품 단위 split에 필수.
    """
...
def split_by_origin(pairs: list[dict], rng: random.Random) -> tuple[list[dict], list[dict], list[dict]]:
    """작품 단위 90/5/5 split. 같은 작품의 passage가 splits 사이에 섞이지 않음.

    **doc_type별 stratified greedy** — val/test에 각 doc_type이 모두 들어가도록.
    (drama_mining/data/split.py의 동일 픽스 패턴)
    """
...
def _len_stats(values: list[int]) -> dict:
    if not values:
        return {"n": 0, "min": 0, "mean": 0, "median": 0, "max": 0}
    return {
        "n": len(values),
        "min": min(values),
...
def main() -> int:
    ...
    print(f"[stage1.1] reading {TL1_ZIP}", file=sys.stderr, flush=True)
    pairs = stream_pairs()
    ...
    summary_lens = [len(r["summary1"]) for r in pairs]
    ...
    stats = {
        "schema_version": "parse_stats_v1",
        "seed": SEED,
        "source_zip": str(TL1_ZIP.relative_to(ROOT)).replace("\\", "/"),
        "doc_types_included": list(DOC_TYPES),
        ...
        "summary1_length_chars": _len_stats(summary_lens),
    ...
    print(f"[stage1.1] wrote {OUT_DIR}", file=sys.stderr, flush=True)
    return 0
