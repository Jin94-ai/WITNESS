"""ONESHOT Gemma 4 E4B labeling — Stage 1.6 (final prompt lock).

Per Stage 1.6 Final Prompt Lock Directive.

Same 10 passages as Stage 1 / 1.5 (seed=42). All Stage 1.5 prompt fixes preserved +
1 new fix:
  5. unknown taxonomy leakage 차단 — "unknown"은 conflict_axis 전용.
     primary_pressures / secondary_pressures / primary_desires / secondary_desires에
     절대 "unknown" 금지.

Run:
    python -m scripts.labeling.oneshot_label_with_gemma_stage1_6
"""
...
# --- System prompt (Stage 1.5 + Stage 1.6 unknown leakage block) ---
...
def run_stage1_6() -> dict:
    ...
    private_path = PRIVATE_DIR / "work_e46069c4b4_stage1_6_private.jsonl"
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = run_stage1_6()
    print(json.dumps(summary, ensure_ascii=False, indent=2))
    # Save JSON summary for downstream report generator
    (PUBLIC_DIR / "stage1_6_summary.json").write_text(
