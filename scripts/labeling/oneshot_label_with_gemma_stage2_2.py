"""ONESHOT Gemma 4 E4B labeling — Stage 2.2 (prompt v2).

Per stage2_qualitative_review.md §8 4 권장.

Same 50 passages as Stage 2 (seed=42, temp=1.0). Prompt v2 변경:
  (a) PS/AV 사용 조건 엄격화 (외부 공권력/사회적 시선 명시 시만)
  (b) 다른 pressure (group_tension / blame_concentration / shame_self / confusion) 우선순위 강조
  (c) survival 사용 조건 엄격화 (물리적/경제적 생존 위협 명시 시만) + Desire 선택 기준 신규
  (d) love → desire 차단 강화 (Few-shot 예시에 love-as-pressure 케이스 추가)

System prompt는 Stage 1.6 LOCK 그대로 유지.

Run:
    python -m scripts.labeling.oneshot_label_with_gemma_stage2_2
"""
...
def run_stage2_2() -> dict:
    ...
    print(f"[stage2_2] {len(sampled)} sampled, overlap with stage1.6: {len(overlap)}/10", file=sys.stderr)

    print(f"[stage2_2] warming up gemma4:e4b...", file=sys.stderr)
    t_wu = time.time()
    label_passage("테스트 씬: 두 사람이 말없이 서 있다.")
    print(f"[stage2_2] warmup {time.time()-t_wu:.1f}s", file=sys.stderr)

    private_path = PRIVATE_DIR / "work_e46069c4b4_stage2_2_private.jsonl"
    ...
    with private_path.open("w", encoding="utf-8") as fp:
        for i, p in enumerate(sampled, 1):
            t_start = time.time()
            print(f"[stage2_2] {i}/{STAGE_SAMPLE_SIZE}: {p['passage_id']} (len={p['passage_length']})", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = run_stage2_2()
    print(json.dumps(summary, ensure_ascii=False, indent=2))
    (PUBLIC_DIR / "stage2_2_summary.json").write_text(
