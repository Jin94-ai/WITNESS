"""ONESHOT Gemma 4 E4B labeling — Stage 1.5 (prompt fix re-run).

Per docs/WITNESS_GEMMA_LABELING_STAGE1_5_DIRECTIVE.md.

Same 10 passages as Stage 1 (seed=42, same extraction logic), with 4 prompt fixes:
  1. love taxonomy 충돌 해결 (love는 Pressure만)
  2. Confidence calibration (5 tiers + 복합 장면 0.6 이하)
  3. 과다 라벨 방지 (public_suspicion / authority_vigilance / blame_concentration / group_tension 사용 기준)
  4. 복합 passage 처리 (multiple scenes → strongest pressure + unknown 허용)

Run:
    python -m scripts.labeling.oneshot_label_with_gemma_stage1_5
"""
...
# --- System prompt (directive §2 fix added) ---
...
# --- User prompt (directive §2-5 fixes applied) ---
...
def run_stage1_5() -> dict:
    ...
    # Same seed + same sampling logic as Stage 1 → same 10 passages
    rng = random.Random(RANDOM_SEED)
    sample_pool = sorted(all_passages, key=lambda e: e["passage_id"])
    sampled = rng.sample(sample_pool, STAGE_SAMPLE_SIZE)

    print(f"[stage1.5] warming up gemma4:e4b...", file=sys.stderr)
    t_wu = time.time()
    label_passage("테스트 씬: 두 사람이 말없이 서 있다.")
    print(f"[stage1.5] warmup {time.time()-t_wu:.1f}s", file=sys.stderr)

    private_path = PRIVATE_DIR / "work_e46069c4b4_stage1_5_private.jsonl"
    ...
    with private_path.open("w", encoding="utf-8") as fp:
        for i, p in enumerate(sampled, 1):
            t_start = time.time()
            print(f"[stage1.5] {i}/{STAGE_SAMPLE_SIZE}: {p['passage_id']} (len={p['passage_length']})", file=sys.stderr)
            ...
            if "_error" in label:
                ...
                if "missing_fields" in issues:
                    schema_missing += 1
                if any(k in issues for k in ("invalid_pressures", "invalid_desires", "invalid_conflict_axis")):
                    taxonomy_invalid += 1
                if not issues:
                    success += 1
    ...
    # Aggregate stats for public report
    confidences = [r["label"].get("confidence") for r in private_records if "_error" not in r["label"]]
    conf_unique = sorted({float(c) for c in confidences if c is not None})
    ...
    summary = {
        ...
        "confidence_distribution": dict(Counter([float(c) for c in confidences if c is not None])),
...
def main(argv: list[str] | None = None) -> int:
    ...
    summary = run_stage1_5()
    print(json.dumps(summary, ensure_ascii=False, indent=2))
    # Save JSON summary for downstream report generator
    (PUBLIC_DIR / "stage1_5_summary.json").write_text(
