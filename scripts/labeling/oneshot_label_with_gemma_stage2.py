"""ONESHOT Gemma 4 E4B labeling — Stage 2 (50 passages, locked prompt).

Per Stage 2 Directive.

Stage 1.6 lock prompt 그대로 사용 (love/unknown 차단).
50 passages from same work (work_e46069c4b4), seed=42, deterministic.
Stage 1.6 10개 passage_ids와의 overlap을 report에 명시.

Hard fail validation:
  - "unknown" in any pressures/desires → invalid
  - "love" in any desires → invalid
  - any taxonomy value not in ALLOWED set → invalid

Run:
    python -m scripts.labeling.oneshot_label_with_gemma_stage2
"""
...
def run_stage2() -> dict:
    ...
    print(f"[stage2] {len(sampled)} sampled, overlap with stage1.6: {len(overlap)}/10", file=sys.stderr)

    print(f"[stage2] warming up gemma4:e4b...", file=sys.stderr)
    t_wu = time.time()
    label_passage("테스트 씬: 두 사람이 말없이 서 있다.")
    print(f"[stage2] warmup {time.time()-t_wu:.1f}s", file=sys.stderr)

    private_path = PRIVATE_DIR / "work_e46069c4b4_stage2_private.jsonl"
    ...
    with private_path.open("w", encoding="utf-8") as fp:
        for i, p in enumerate(sampled, 1):
            t_start = time.time()
            print(f"[stage2] {i}/{STAGE_SAMPLE_SIZE}: {p['passage_id']} (len={p['passage_length']})", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    ...
    (PUBLIC_DIR / "stage2_summary.json").write_text(
