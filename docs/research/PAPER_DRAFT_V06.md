...
Agent-based simulations of historical biographical events face a central validation problem: when models generate plausible outcomes, how do we distinguish genuine emergent causality from well-tuned pattern matching? We present **Witness**, a multi-agent, hazard-driven simulator that models two distinct scenarios under a shared symbolic engine — Peter (4 agents, 500 ticks; a passion narrative) and Van Gogh (3 agents, 150 ticks; the Arles period). On top of this engine we introduce a **seven-layer validation framework** combining pattern-oriented modeling (POM), counterfactual ablation, event-relative checkpointing, partial holdout forecasting, explanation faithfulness via per-agent removal, cross-scenario isomorphism via a Kolmogorov–Smirnov (KS) test, and behavioral rate regression. Seven findings emerge: (1) arrest-class events appear spontaneously in 100% of runs without hardcoded timing; (2) the Judas disillusionment variable functions as a dominant proximal driver (Cohen's *d* = -6.87; permutation *p* < 0.001); (3) behavioral rate signals outperform state signals as leading indicators at early horizons; (4) both scenarios exhibit a single rare-action bottleneck pattern (Phi > 0.95) — *sword_drawn* in Peter, *self_harm* in Van Gogh — suggesting structural isomorphism; (5) normalized event-timing distributions differ significantly across scenarios (KS *D* = 0.567, *p* < 0.01), supporting a **dual-layer hypothesis** in which deep behavioral structure is shared while surface dynamics are scenario-specific; (6) world-layer configuration (cast composition, placement, event density, event count) shifts outcome distributions within a single scenario, with mean per-dimension modal-flip ratio of 39% under 5-seed ensemble, and two scenario-locked unanimous attractors detectable across 60 runs (Appendix G); and (7) the annotated probe representation admits a deterministic Korean narrative renderer (Appendix H) such that the configuration sensitivity of finding (6) is observable at the prose level — saturation versus recovery cohorts produce tonally opposite paragraphs without language-model invocation. A methodological by-product: a cross-seed re-test of finding (6) revealed that single-seed snapshots can bias per-dimension sensitivity ratios by ±33 percentage points in either direction, and we recommend ensemble characterization as the default for ABM sensitivity claims (§7.4). We release the framework as a research prototype, validated across 1500+ unit and integration tests, and position it as groundwork for subsequent learned-drive extensions and the v2.0 narrative witness layer.
...
### 6.9 Configuration sensitivity at the world layer (Branch C)

Holding scenario, role roster, and engine code fixed, we vary four world-layer dimensions — initial agent placement, cast composition (which roles are present), event density, and event count × spacing — and ask whether the final-summary outcome distribution shifts. Under a 5-seed ensemble across 36 generated probes (9 per dimension), the per-dimension modal-flip ratios are 44%, 56%, 44%, and 11% respectively (Appendix G). The cast-composition dimension is the most ensemble-stable contributor; the scarcity-event-count dimension is the least, and a striking nonmonotonic pattern at seed=0 (three accusations producing recovery while two produce saturation) attenuates in the cross-seed view. The qualitative finding — world-layer configuration changes the outcome distribution within a single scenario — survives ensemble correction; the specific ranking and magnitudes do not. Two unanimous (5/5 seeds) cells appear in the cross-scenario test of Appendix G: accusation under widely-spaced events always produces saturation, sacred under tightly-clustered miracle events always produces recovery. These read as scenario-locked attractors of the world layer, not as universal mechanisms.

### 6.10 Narrative renderability of simulator outputs

The annotated probe representation introduced for the readability protocol (Appendix B) admits a deterministic, template-guided Korean narrative renderer with no language-model invocation (Appendix H). Across all 48 probes — 12 baseline plus 36 from §6.9 — the renderer produces summary-form (400–800 character) and narrative-form (700–1300 character) outputs that satisfy a six-criterion acceptance protocol: (1) story flow identifiable in 12 of 12 baseline probes; (2) outcome categories distinguishable in tone; (3) crowd-mood, authority, public-attention, interpersonal axes all surfaced; (4) reads as narrative rather than report; (5) probe-level variation visible; (6) no template repetition under hashed sentence-pool variation. An automated forbidden-phrase audit (`scripts/audit_report.py --stories`) confirms that none of the 96 generated files (48 probes × two output forms) leak raw IDs, raw numbers, or meta-language back into the prose. The §6.9 nonmonotonic and placement-reversal findings appear as readable narrative contrasts at the prose level — not just as numerical contrasts in the analysis layer — supporting the project's longer-term goal of a witness-mode simulator output.
...
- Configuration sensitivity (§6.9) is reported at modal level under a 5-seed ensemble; per-cell within-scenario variance is non-trivial (most cells show 2-3 distinct outcomes across 5 seeds) and a finer characterization of distributional signatures awaits larger-N ensembles.
...
### 7.4 Single-seed inadequacy for sensitivity claims

The Branch C cross-seed re-test (Appendix G) revealed that single-seed snapshots can bias per-dimension sensitivity ratios by ±33 percentage points in *either* direction — over- or under-stating the effect. In our 36-probe set the scarcity-depth dimension dropped from 44% to 11% (overstated by seed=0), while the event-density dimension *increased* from 22% to 44% (understated by seed=0); the relative ranking of dimensions inverted in one slot. We therefore treat single-seed runs as illustrations and require 5+ seed ensembles for any headline sensitivity claim. The HARNESS discipline §H8 ("single-seed conditioning warns") is the project-internal codification of this; we recommend it as a default for ABM validation studies more generally.
...
## Appendix G — Branch C 1차 evidence (world engine cross-seed analysis, 2026-04-28)

**Context.** Following GPT-5.5 blind eval in mid-April 2026, the paper team initiated a `Branch C` exploration of world-engine-level configuration sensitivity. Branch C does not extend the engine kernel; instead, it varies generator-level inputs (cast composition, spatial placement, event density, event count × spacing) to probe whether the §5/§6 findings are configuration-robust or configuration-modulated. All variants are non-destructive: engine code is untouched, ABSOLUTE Rule #6 preserved.

**Slice design (4 dimensions × 9 cells each = 36 annotated probes).** S5 placement (3 scenarios × {original, inverted, clustered}). S4 cast composition (3 scenarios × {full, no_authority, no_outsider}). S3 event density (sacred only, 3 miracle counts × 3 spacings). S2 scarcity depth (scarcity only, 3 event counts × 3 crowd densities). All probes use seed=0 in the initial pass.

**Initial finding (seed=0)**. Per-dimension configuration sensitivity (probes flipping baseline final-summary outcome): S5 67%, S4 67%, S3 22%, S2 44%. Mean 50%. Combined 18/36 flips. Five distinct outcome labels (LOW_ACTIVITY / RECOVERY / SATURATION / MIXED / PARTIAL) appear across the 36-probe set. Q2a (primary pressure) detection 36/36 = 100% — configuration-invariant.

**Hypothesis cycle (S2 nonmonotonicity).** Within S2, scarcity event count produced a striking nonmonotonic pattern at seed=0: single accusation → SATURATION (3/3 high-density), double → SATURATION (3/3), triple → RECOVERY (3/3). Three candidate hypotheses (forgiveness cascade scaling; moral fatigue; cohort propagation) were rejected by per-probe trace inspection. A fourth hypothesis D was proposed (oscillation-enabled confession): well-spaced events produce shame oscillation valleys, which enable more confessions, which drive recovery. A spread/cluster falsification test showed mild-cluster (gap 2-5 ticks) → SATURATION but very-cluster (consecutive ticks) → PARTIAL — suggesting D' (D generalized) is at minimum two-regime, not single-mechanism.

**Generalization test (D' across scenarios).** Same 4 spacing variants applied to accusation and sacred scenarios. Modal outcomes diverged: scarcity spread → RECOVERY, accusation spread → SATURATION (5/5 across seeds), sacred spread → PARTIAL. D' rejected as a universal mechanism. Each scenario carries its own spacing-to-outcome mapping; the dynamics-rule itself is scenario-specific.

**Cross-seed walkback (HARNESS H1 + H4 self-falsification).** All Branch C 1차 evidence used seed=0 only. A 5-seed ensemble re-test (180 runs, seeds 0-4) produced significant shifts in per-dimension sensitivity:

| Slice | Seed=0 | 5-seed ensemble | Δ |
|---|---:|---:|---:|
| S5 placement | 67% | 44% | -23pp |
| S4 cast composition | 67% | 56% | -11pp |
| S3 event density | 22% | 44% | **+22pp (increase)** |
| S2 scarcity depth | 44% | 11% | **-33pp (collapse)** |
| **Mean** | 50% | 39% | -11pp |

Two findings are non-trivial: (1) seed=0 is biased in unpredictable directions, not systematically optimistic — S3 sensitivity was *underestimated* by 22pp, while S2 was overestimated by 33pp. (2) The S2 nonmonotonicity (LOOP 69 finding) is largely a seed=0 artifact under the 5-seed ensemble; only 1/9 cells flips robustly. Within-cell variance: S3 has 4/9 unanimous cells (most ensemble-stable), S4 and S2 have 0/9 unanimous (most variance).

**What survives ensemble correction (claims robust at modal level):**
- WITNESS dynamics are configuration-dependent: 3 of 4 dimensions show ≥44% ensemble sensitivity.
- Scenarios have distinct distributional signatures (D' rejection robust): 3 of 4 spacings show distinct modal outcomes across scenarios.
- Cast variation is the most ensemble-robust dimension.
- Two scenario-locked unanimous patterns: accusation/spread → SATURATION (5/5), sacred/very-cluster → RECOVERY (5/5).
- Top blame target (v4 generator-level surfacing of `c.blame_concentration[target]`) is deterministic per scenario: scarcity 100% fisher_laborer; accusation 82% crowd_participant; sacred 70% disciple_follower.
- Q2a-typing 36/36 across the original probes; configuration-invariant.

**Methodological lesson.** Single-seed ensemble snapshots are inadequate for sensitivity claims even at the relative-comparison level. The S2 dimension went from 4th-most-sensitive at seed=0 to least-sensitive under ensemble correction — a complete relative-rank reversal from a single-seed measurement. Future world-engine validation studies must use 5+ seed ensembles for headline sensitivity ratios; single-seed runs may be reported as illustrations but not as quantitative claims. This finding has been incorporated into HARNESS (H1, H4) discipline and `lessons.md` L13.

**Files.** `docs/b_direction/BRANCH_C_FIRST_EVIDENCE_SUMMARY.md` (v4.4, 36-probe summary with cross-seed correction); `docs/b_direction/BRANCH_C_CROSS_SEED_ENSEMBLE_RESULTS.md` (per-cell modal outcomes, 180 runs); `docs/b_direction/LEE_GATE_2026-04-28_BRANCH_C.md` (decision package). Generators: `scripts/b_direction/generate_{placement,cast,event_density,scarcity_depth}_variations.py`. Cross-seed scripts: `scripts/b_direction/test_{s5,d_prime,all_slices}_cross_seed.py`. Validation: `scripts/b_direction/validate_annotated_v4.py` confirms 48/48 probes (12 baseline + 36 Branch C) carry the v4 annotated field set.

**Status.** Branch C 1차 evidence locked at v4.4. Lee gate open for: (a) lock + send 36 probes to GPT-5.5 with explicit seed=0 disclosure; (b) S6 engine-touch (authority autonomy, KERNEL_GAPS Gap 6) — requires Lee directive; (c) accept walkback into paper §6 as ensemble-corrected ratios. Authors note that the cross-seed walkback is itself a contribution to ABM validation methodology — falsification cycles surfaced via HARNESS H1/H4 discipline are reproducible and reportable.

---

## Appendix H — Story Output Layer (Korean narrative renderer, 2026-04-28)

**Context.** The simulator's primary outputs to date have been numerical (POM scorecards, KS distances, Cohen's *d* effect sizes) and structured (annotated probes, trajectory JSONL streams). For the project's stated long-term goal — a witness-mode simulator in which a human reader experiences the unfolding of historical events as narrative — those outputs are necessary but not sufficient. Numerical and structured outputs are *describable* by an analyst but not *readable* by a witness. Appendix H reports a story output layer that converts annotated probes into Korean narrative text, intended as the entry point to the v2.0 Narrative Witness Layer described in §8.

**Pipeline (three stages).** The renderer is template-guided, deterministic, and does not invoke any language model. Each annotated probe passes through three scripts in sequence:

1. *Extraction* (`scripts/story/extract_story_features.py`). Regex-based parser pulls the structured fields named in §6 (final summary, primary pressure, failure mode, cohort outcomes, accusation count, confession count, forgiveness count, top blame target, public suspicion peak/final, authority vigilance peak/final, role roster, locations, first 30 events from the event log) into a JSON record at `data/story/story_features/{probe_id}.json`. The extractor handles both the original 12 baseline probes (`P{1-12}_ANNOTATED.txt`) and the 36 Branch C variant probes (`P_PV_NN.txt` placement, `P_CV_NN.txt` cast, `P_ED_NN.txt` event density, `P_S2_NN.txt` scarcity depth) under a single `_resolve_probe_path()` dispatch.

2. *Narrative IR construction* (`scripts/story/build_narrative_ir.py`). Numeric features are converted into semantic atoms — `blame_band ∈ {absent, weak, strong, dominant}`, `confession_volume ∈ {low, moderate, high}` (scenario-normalized for sacred), `authority_pattern ∈ {absent, decayed, loosened, sustained}`, `top_blame_strong: bool`, scenario-specific `location_name` mapping (e.g. for scarcity probes, L1→곡물 창고, L2→빈민가, L3→시장) — and assembled into a five-field narrative IR (`world_opening`, `initial_tension`, `pressure_arc`, `group_response`, `turning_point`, `outcome`, `world_aftereffect`). The IR is the unit at which story-quality issues are diagnosed and patched: a saturation-classified probe with insufficient blame strength is a Stage 2 (IR) bug, while a saturation IR rendered with recovery-toned Korean is a Stage 3 (renderer) bug.

3. *Korean rendering* (`scripts/story/render_story_ko.py`). Each IR atom maps to one or more Korean sentence templates. Two output types are produced: a "summary" (400–800 characters, four to six paragraphs, dry chronicle voice) and a "narrative" (1000–1800 characters target, five-stage structure: opening → pressure rise → response branching → resolution → aftermath, mildly emotional voice). The renderer enforces a forbidden-phrase filter (no `trajectory`, `probe`, `final summary`, raw IDs `P6 / A1 / L1`, raw numbers `peak X.XX / t=N`, or meta phrases such as `이 시뮬레이션에서는`). Korean particle selection is automated by a 받침 (final-consonant) detector to avoid `을(를)` artifacts. Per-probe variation is induced via a deterministic `variant_pick(probe_id, slot, pool)` hash so that two annotated probes that compress to the same IR (e.g., baseline probes P4 and P5, both sacred RECOVERY with identical cohort structure) still produce distinct opening and outcome sentences.

**Acceptance results (MVP Phase 2).** The acceptance protocol uses six criteria adapted from the readability blind eval (Appendix B): (1) story flow identifiable in ≥9 of 12 probes; (2) recovery / saturation / mixed distinguishable in the prose; (3) ≥2 of crowd-mood / authority / public-attention / interpersonal axes surfaced; (4) reads as narrative rather than report; (5) probe-level variation visible in text; (6) no obvious template repetition. The first version of the renderer passed five criteria with one marginal (P4 and P5 produced identical text, since their IRs were identical). After introducing the `variant_pick()` hash and stage-to-stage transition pools (TRANSITION_TO_PRESSURE, TRANSITION_TO_RESPONSE, TRANSITION_TO_OUTCOME, TRANSITION_TO_AFTEREFFECT, three variants each), the same-IR pair P4 and P5 produced different first paragraphs, and the sixth criterion passed. Final verdict: 6/6.

**Branch C × Story integration.** When the same pipeline is applied to the 36 Branch C variant probes (Appendix G), three findings of the world-engine analysis surface in the Korean prose without further intervention: (i) the unique LOW_ACTIVITY case — `P_PV_09`, sacred scenario with all agents clustered in the inner court — opens with "특별한 일이 없는 날이었다 (it was a day on which nothing in particular happened)" and closes with "사건이라 부를 만한 일도 없이, 시간은 흘러갔다 (without anything that could be called an event, the time passed)", reading correctly as a non-event in tone rather than as missing data; (ii) the placement-reversal pair `P_PV_01` (RECOVERY) versus `P_PV_02` (SATURATION), which differ only in initial agent placement, produce opposite endings — "사람들은 흔들렸지만 다시 자리를 잡았다" versus "사람들은 자리에 굳었다" — showing that the Branch C finding "placement inversion reverses outcome" survives translation to narrative; (iii) the scarcity-depth nonmonotonic pair `P_S2_05` (two accusations, SATURATION) versus `P_S2_08` (three accusations, RECOVERY) produces tonally opposite closing paragraphs — the heavier event count results in the lighter outcome — surfacing the LOOP 69 nonmonotonicity as a directly readable narrative contrast. A reader of P_S2_05 and P_S2_08 in sequence is led to ask "why does the more-burdened world recover while the less-burdened one stays stuck?" — a question Appendix G's mechanism analysis (hypothesis D, oscillation-enabled confession) addresses but which the prose makes accessible without requiring numerical inspection.

**What this layer does not do.** The renderer does not infer character interiority that the simulator did not produce — no "Peter felt afraid as he watched the courtyard" lines unless the corresponding state variables are in the trace. It does not invent events, locations, or agents; the role roster and location set come strictly from the annotated probe. It does not reach paragraph-level fluency comparable to a human author writing freely; the templates are deliberately narrow, because a wider sentence pool would require either an LLM (rejected for reproducibility) or a much larger hand-authored template library. It does not pretend to be a finished player-facing experience — it produces text that a researcher or designer can read to see whether the simulator's outcome distribution has narrative shape, not text intended for a player.

**Limitations and the path to v2.0.** The core limitation is that the IR is too compressed: two probes whose simulations differed substantially can collapse to the same IR if their compressed annotated form is identical, and at that point only the `variant_pick()` hash distinguishes their stories. A v2.0 narrative layer would either route richer fields (per-cohort confession timing rhythm, per-agent emotional state at ten-tick checkpoints) into the IR, or operate directly on the trace stream at the player-view filter level. The current MVP renders 48 baseline-and-Branch-C probes without exposing any quality regression, but its growth ceiling is set by what the annotated probe captures, not by the renderer itself. Future work — once the renderer's downstream consumer is more clearly defined (a static text export, an interactive viewer, a player simulator) — will extend the IR to include trajectory-level features.

**Files.** Spec: `docs/story/STORY_OUTPUT_SPEC.md`. Phase 2 acceptance and progress: `docs/story/STORY_MVP_ACCEPTANCE_v2.md`, `docs/story/STORY_PHASE2_PROGRESS.md`. Branch C integration analysis: `docs/story/STORY_BRANCH_C_INTEGRATION.md`. Curated reading list of 6 representative cases: `docs/story/STORY_HIGHLIGHTS.md`. Generated outputs (96 files: 48 summaries + 48 narratives): `docs/story/generated/`. Pipeline scripts: `scripts/story/{extract_story_features,build_narrative_ir,render_story_ko}.py`. The full pipeline runs in ~5 seconds for all 48 probes on a reference workstation; no engine, content pack, or annotated probe specification was modified.
